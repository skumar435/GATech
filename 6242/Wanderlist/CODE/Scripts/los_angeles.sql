...
CREATE SCHEMA `los_angeles` ;
CREATE TABLE `los_angeles`.`restaurants_rawdata` (
...
CREATE VIEW `los_angeles`.`restaurants_view` AS SELECT id, name, rating, price, review_count, category,
...
FROM `los_angeles`.`restaurants_rawdata`;


-- create tables for hotel data---------------------------------


CREATE TABLE `los_angeles`.`hotel_rawdata` (
...
CREATE VIEW `los_angeles`.`hotel_view` AS
...
FROM `los_angeles`.`hotel_rawdata`; 



-- create tables for places of interest data---------------------------------

CREATE TABLE `los_angeles`.`poi` (
...
CREATE VIEW `los_angeles`.`poi_view` AS SELECT DISTINCT * FROM `los_angeles`.`poi`;

-- create tables for city data---------------------------------

CREATE TABLE `los_angeles`.`city_data` (city_name VARCHAR(50), city_code VARCHAR(50), places_id VARCHAR(50));

-- create tables for flight data---------------------------------

CREATE TABLE `los_angeles`.`prices_flights` (min_price INT, is_direct VARCHAR(10), date_inbound DATE, date_outbound DATE, carrier_id VARCHAR(50),
origin_id VARCHAR(50), dest_id VARCHAR(50));

-- create tables for carrier data---------------------------------

CREATE TABLE `los_angeles`.`carrier_data` (carrier_id VARCHAR(50), carrier_name VARCHAR(50));

-- Data Import for above three tables------------------------

-- data clean up and view setup-----------------------------

CREATE VIEW `los_angeles`.`flight_view` AS
SELECT a.is_direct, a.date_inbound, a.date_outbound,
a.min_price, b.city_name as source_city, c.carrier_name
FROM `los_angeles`.`prices_flights` AS a, `los_angeles`.`city_data` AS b, `los_angeles`.`carrier_data` AS c
