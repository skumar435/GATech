...
CREATE SCHEMA `atlanta` ;

CREATE TABLE `atlanta`.`restaurants_rawdata` (
...
CREATE VIEW `atlanta`.`restaurants_view` AS SELECT id, name, rating, price, review_count, category,
...
FROM `atlanta`.`restaurants_rawdata`;

-- create tables for hotel data---------------------------------

CREATE TABLE `atlanta`.`hotel_rawdata` (
...
CREATE VIEW `atlanta`.`hotel_view` AS
...
FROM `atlanta`.`hotel_rawdata`; 

-- create tables for places of interest data---------------------------------

CREATE TABLE `atlanta`.`poi` (
...
CREATE VIEW `atlanta`.`poi_view` AS SELECT DISTINCT * FROM `atlanta`.`poi`;


-- create tables for city data---------------------------------

CREATE TABLE `atlanta`.`city_data` (city_name VARCHAR(50), city_code VARCHAR(50), places_id VARCHAR(50));

-- create tables for flight data---------------------------------
DROP TABLE `atlanta`.`atl_prices_flights`;
CREATE TABLE `atlanta`.`prices_flights` (min_price INT, is_direct VARCHAR(10), date_inbound DATE, date_outbound DATE, carrier_id VARCHAR(50),
origin_id VARCHAR(50), dest_id VARCHAR(50));

-- create tables for carrier data---------------------------------

CREATE TABLE `atlanta`.`carrier_data` (carrier_id VARCHAR(50), carrier_name VARCHAR(50));

-- Data Import for above three tables------------------------

-- data clean up and view setup-----------------------------

CREATE VIEW `atlanta`.`flight_view` AS
SELECT a.is_direct, a.date_inbound, a.date_outbound,
a.min_price, b.city_name as source_city, c.carrier_name
FROM `atlanta`.`prices_flights` AS a, `atlanta`.`city_data` AS b, `atlanta`.`carrier_data` AS c
