...
CREATE SCHEMA `philadelphia` ;

CREATE TABLE `philadelphia`.`restaurants_rawdata` (
...
CREATE VIEW `philadelphia`.`restaurants_view` AS SELECT id, name, rating, price, review_count, category,
...
FROM `philadelphia`.`restaurants_rawdata`;


-- create tables for hotel data---------------------------------


CREATE TABLE `philadelphia`.`hotel_rawdata` (
...
CREATE VIEW `philadelphia`.`hotel_view` AS
...
FROM `philadelphia`.`hotel_rawdata`; 


-- create tables for places of interest data---------------------------------

CREATE TABLE `philadelphia`.`poi` (
...
CREATE VIEW `philadelphia`.`poi_view` AS SELECT DISTINCT * FROM `philadelphia`.`poi`;

-- create tables for city data---------------------------------

CREATE TABLE `philadelphia`.`city_data` (city_name VARCHAR(50), city_code VARCHAR(50), places_id VARCHAR(50));

-- create tables for flight data---------------------------------

CREATE TABLE `philadelphia`.`prices_flights` (min_price INT, is_direct VARCHAR(10), date_inbound DATE, date_outbound DATE, carrier_id VARCHAR(50),
origin_id VARCHAR(50), dest_id VARCHAR(50));

-- create tables for carrier data---------------------------------

CREATE TABLE `philadelphia`.`carrier_data` (carrier_id VARCHAR(50), carrier_name VARCHAR(50));

-- Data Import for above three tables------------------------

-- data clean up and view setup-----------------------------

CREATE VIEW `philadelphia`.`flight_view` AS
SELECT a.is_direct, a.date_inbound, a.date_outbound,
a.min_price, b.city_name as source_city, c.carrier_name
FROM `philadelphia`.`prices_flights` AS a, `philadelphia`.`city_data` AS b, `philadelphia`.`carrier_data` AS c
