...
CREATE TABLE `miami`.`restaurants_rawdata` (
...
CREATE VIEW `miami`.`restaurants_view` AS SELECT id, name, rating, price, review_count, category,
...
FROM `miami`.`restaurants_rawdata`;
...
CREATE TABLE `miami`.`hotel_rawdata` (
...
CREATE VIEW `miami`.`hotel_view` AS
...
FROM `miami`.`hotel_rawdata`; 


-- create tables for places of interest data---------------------------------

CREATE TABLE `miami`.`poi` (
...
CREATE VIEW `miami`.`poi_view` AS SELECT DISTINCT * FROM `miami`.`poi`;

-- create tables for city data---------------------------------

CREATE TABLE `miami`.`city_data` (city_name VARCHAR(50), city_code VARCHAR(50), places_id VARCHAR(50));

-- create tables for flight data---------------------------------

CREATE TABLE `miami`.`prices_flights` (min_price INT, is_direct VARCHAR(10), date_inbound DATE, date_outbound DATE, carrier_id VARCHAR(50),
origin_id VARCHAR(50), dest_id VARCHAR(50));

-- create tables for carrier data---------------------------------

CREATE TABLE `miami`.`carrier_data` (carrier_id VARCHAR(50), carrier_name VARCHAR(50));

-- Data Import for above three tables------------------------

-- data clean up and view setup-----------------------------

CREATE VIEW `miami`.`flight_view` AS
SELECT a.is_direct, a.date_inbound, a.date_outbound,
a.min_price, b.city_name as source_city, c.carrier_name
FROM `miami`.`prices_flights` AS a, `miami`.`city_data` AS b, `miami`.`carrier_data` AS c
